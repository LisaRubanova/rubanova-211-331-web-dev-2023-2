...
lable = input('Введите имя устройства:')
key = london_co.get(lable)
key = list(key.keys())
key = (', ').join(key)


lable2 = input(('Введите имя параметра ({}):').format(key)).lower()

print(london_co.get(lable).get(lable2, 'Такого параметра нет'))